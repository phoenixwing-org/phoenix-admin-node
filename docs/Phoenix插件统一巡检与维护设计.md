...
推荐继续以 `/phoenix/plugins` 作为统一入口，不再增加一套只服务开发挂载的长期页面。页面按
`moduleId` 展示一张插件卡片，卡片展开后按 Block 显示：
...
| 巡检状态 | 说明 | 允许动作 |
| --- | --- | --- |
| `mounted-unregistered` | 双端开发挂载合法，但 Pah 无台账 | 选择与挂载逐字节一致的不可变包 |
...
巡检可以报告依赖状态，但 Host 不执行插件提供的任意 `pnpm` 命令：
