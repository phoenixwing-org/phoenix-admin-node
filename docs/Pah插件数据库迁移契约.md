...
本地可信备份会在写入备份前统一检查 `psql`、`pg_dump`、`pg_restore`、`createdb`
和 `dropdb`，并要求所有客户端主版本与实际服务端一致。Windows 的绝对 `bin` 路径、
五个独立可执行文件配置、脱敏错误语义及 R14/R15 复检步骤见
[Windows可信备份工具链.md](Windows可信备份工具链.md)。配置主版本不能代替真实
`psql` 探测，也不能绕过恢复演练。
